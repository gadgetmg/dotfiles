{inputs, ...}: {
  flake.modules.nixos.wolf = {config, ...}: {
    ...
    services = {
      ...
      in {
        enable = true;
        image = "ghcr.io/gadgetmg/wolf:stable";
        openFirewall = true;
        config = {
          ...
          gstreamer = {
            audio = {
              default_audio_params = "queue max-size-buffers=2 leaky=downstream ! audiorate ! audioconvert";
              ...
              '';
              default_source = "interpipesrc name=interpipesrc_{}_audio listen-to={session_id}_audio is-live=true stream-sync=restart-ts max-bytes=0 max-buffers=2 block=false";
            };
            video = {
              default_sink = ''
